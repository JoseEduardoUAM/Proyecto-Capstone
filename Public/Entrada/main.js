...
socket.on( 'respuestaVerificarQREntrada' , ( data ) => {
    if( data.error ){
        ...
        Swal.fire({ icon: 'success', title: `Bienvenido ${data.msg}`, showConfirmButton: false, timer: 2000 })
            .then(() => {
                scanner.start()
            })
    }
...
function enviarCodigoQR( data ){
    ...
    if( qrCode.codeSecret ){
        scanner.stop()
        socket.emit( 'RegistrarEntrada' , qrCode )
    }
}
